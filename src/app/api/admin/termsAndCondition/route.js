import termsAndCondtion from "@/models/termsAndCondtion";
...
export async function POST(req) {
    ...
    try {
        ...
        // const policyExist = await termsAndCondtion.findOne({heading:title});

        // if(policyExist){
        //     return Response.json({ message: 'policy Already Exist ' } , {status:401});
        // }

        const newpolicy = new termsAndCondtion({heading:title,description:description});

        const savedpolicy = await newpolicy.save();

        const policies = await termsAndCondtion.find();
...
export async function GET(req){
    try {
        ...
        const policy = await termsAndCondtion.find()
...
export async function PUT(req) {
    try {
        ...
        const updatedPolicy = await termsAndCondtion.findOneAndUpdate(
            { _id: _id },
            { heading: title, description: description },
            { new: true }
        );
    
        const allPolicies = await termsAndCondtion.find();
